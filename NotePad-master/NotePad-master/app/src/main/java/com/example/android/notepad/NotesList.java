...
public class NotesList extends ListActivity {
    ...
    /** The index of the title column */ //标题栏索引
    private static final int COLUMN_INDEX_TITLE = 1;

    /**
     * onCreate is called when Android starts this Activity from scratch.
     * 当Android从零开始启动这个活动时调用OnCreate。
     */
    @Override
    protected void onCreate(Bundle savedInstanceState) {
         ...
         * provider URI.
         * 如果在启动该活动的Intent中没有给出数据，则在Intent过滤器匹配主动作时启动该活动。我们应该使用默认提供程序URI。
         */
        // Gets the intent that started this Activity. 获取启动此活动的Intent。
        Intent intent = getIntent();

        // If there is no data associated with the Intent, sets the data to the default URI, which
        // accesses a list of notes.
        //如果没有与Intent相关联的数据，则将数据设置为默认URI，该默认URI访问注释列表。
         ...
         * 为ListVIEW设置上下文菜单激活的回调。侦听器被设置为这个活动。效果是在ListVIEW中为项目启用上下文菜单，而上下文菜单由NoeStEB中的方法处理。
         */
         ...
         * 执行托管查询。该活动在需要时处理关闭和请求CURSORE。
          * 请参阅关于在UI线程上执行提供程序操作的介绍性说明。
         */
        ...
        /*
         * The following two arrays create a "map" between columns in the cursor and view IDs以下两个数组在游标和视图ID中的列之间创建“映射”
         * for items in the ListView. Each element in the dataColumns array represents
         * a column name; each element in the viewID array represents the ID of a View.
         * The SimpleCursorAdapter maps them in ascending order to determine where each column
         * value will appear in the ListView.
         * 下面两个数组在ListVIEW中的列和视图IDS中的列之间创建一个“map”。
         * DATACOLUMNS数组中的每个元素表示一个列名；VIEWD数组中的每个元素表示视图的ID。
         * SimuleCurror适配器以升序映射它们，以确定每个列值将出现在ListVIEW中。
         */
